...
async function checkBusinessIdsInCalls() {
    console.log('Fetching distinct business_ids from "calls"...');
    ...
    if (error) {
        ...
        const ids = new Set(data.map(c => c.business_id).filter(Boolean));
        console.log('Distinct business_ids in calls:', Array.from(ids));
    }
}

checkBusinessIdsInCalls();
